...
internal class GlareSRF : ScriptableRendererFeature
{
    GlareRenderPass pass;
    Material mat;

    //exposed to be tweaked in the renderer settings
    [SerializeField] Shader shader;
    [SerializeField] GlareSetting settings;
    ...
    class GlareRenderPass : ScriptableRenderPass
    {
        ProfilingSampler profileSampler;
        GlareSetting settings;
        Material mat;
        ...
        #region HELPER METHODS
        public bool SetUp(ref Material material, ref GlareSetting setting) //setup the render pass with all the data
        {
            mat = material;
            settings = setting;
        ...
        private void UpdateShaderSettings()
        {
            ...
            mat.SetColor("_Glare_Color", settings.GlareColor);
            mat.SetFloat("_Glare_Amount", settings.GlareAmount);
...
internal class GlareSetting
{
    ...
    public string ProfilerName = "LIGHT GLARE";
    public Color GlareColor;
    [Range(0,1)]public float GlareAmount;
}